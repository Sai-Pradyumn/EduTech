...
@Component({
            ...
            @for (a of apps(); track a.id) {
              <div class="app">
                <div class="flex items-center justify-between gap-2">
                  <span class="min-w-0"><span class="a-role">{{ a.role }}</span><span class="a-co">{{ a.company }}</span></span>
                  <span class="a-score" [style.color]="scoreColor(a.matchScore)">{{ a.matchScore }}</span>
                </div>
                <div class="flex items-center gap-2 mt-1.5">
                  <select class="status" [value]="a.status" (change)="setStatus(a, $event)">
  ...
  styles: [`
    ...
    .app { padding: 9px 11px; border: 1px solid var(--paper-3); border-radius: 11px; background: var(--paper-2); }
...
export class ApplicationsComponent {
  ...
  readonly statuses = ['saved', 'applied', 'interviewing', 'offer', 'rejected'];
