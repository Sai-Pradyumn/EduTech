...
import { MentorSession, MentorSessionDocument, MentorSessionStatus } from './schemas/mentor-session.schema';
import { RequestSessionDto, UpsertMentorProfileDto } from './dto/mentor-marketplace.dto';
...
export class MentorMarketplaceService {
  constructor(
    ...
    @InjectModel(MentorSession.name) private readonly sessions: Model<MentorSessionDocument>,
  ...
  async listMentors(): Promise<MentorView[]> {
    const list = await this.profiles.find().sort({ 'ratingSummary.avg': -1, createdAt: -1 }).limit(60).exec();
  ...
  async upsertProfile(userId: string, dto: UpsertMentorProfileDto): Promise<MentorProfileDocument> {
    const update = {
    ...
    await this.profiles.updateOne({ user: new Types.ObjectId(userId) }, { $set: { user: new Types.ObjectId(userId), ...update } }, { upsert: true }).exec();
    return (await this.myProfile(userId))!;
  }

  // ── sessions ──
  async requestSession(studentId: string, dto: RequestSessionDto): Promise<MentorSessionDocument> {
  ...
  async listSessions(userId: string): Promise<SessionView[]> {
    ...
    const list = await this.sessions.find({ $or: [{ student: uid }, { mentor: uid }] }).sort({ createdAt: -1 }).limit(60).exec();
    return Promise.all(
      list.map(async (s) => {
        const isStudent = String(s.student) === userId;
        const counterpartId = isStudent ? s.mentor : s.student;
        const counterpart = await this.users.findById(String(counterpartId)).catch(() => null);
        return {
          id: String(s._id),
          type: s.type,
          status: s.status,
          ...
          createdAt: (s as MentorSessionDocument & { createdAt?: Date }).createdAt?.toISOString() ?? '',
        };
      }),
    );
  }

  async updateStatus(userId: string, id: string, status: MentorSessionStatus): Promise<MentorSessionDocument> {
    ...
    if (!isMentor && status !== 'cancelled') throw new ForbiddenException('Only the mentor can change this status.');
    session.status = status;
    await session.save();
    if (status === 'completed') {
      const mentor = await this.users.findById(String(session.mentor)).catch(() => null);
  ...
  async addNotes(userId: string, id: string, notes: string): Promise<MentorSessionDocument> {
    const session = await this.ownedAny(userId, id);
    if (String(session.mentor) !== userId) throw new ForbiddenException('Only the mentor can add notes.');
    session.notes = notes;
    await session.save();
    return session;
  }

  private async ownedAny(userId: string, id: string): Promise<MentorSessionDocument> {
    const uid = new Types.ObjectId(userId);
    const session = await this.sessions.findOne({ _id: new Types.ObjectId(id), $or: [{ student: uid }, { mentor: uid }] }).exec();
