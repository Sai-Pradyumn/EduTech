...
export const SESSION_TYPES = ['project_review', 'interview_review', 'roadmap_review', 'portfolio_review', 'general'] as const;
export type MentorSessionType = (typeof SESSION_TYPES)[number];

export const SESSION_STATUS = ['requested', 'accepted', 'completed', 'cancelled'] as const;
...
export class MentorSession {
  @Prop({ type: Types.ObjectId, ref: 'User', required: true, index: true }) mentor!: Types.ObjectId;
  @Prop({ type: Types.ObjectId, ref: 'User', required: true, index: true }) student!: Types.ObjectId;

  @Prop({ type: String, enum: SESSION_TYPES, default: 'general' }) type!: MentorSessionType;
  @Prop({ type: String, enum: SESSION_STATUS, default: 'requested', index: true }) status!: MentorSessionStatus;
