...
import { AddSourceDto, AskSpaceDto, CreateSpaceDto, UpdateSpaceDto } from './dto/space.dto';
...
function toView(s: StudySpaceDocument) {
  return {
    ...
    sources: s.sources.map((src) => ({ id: src.id, type: src.type, title: src.title, text: src.text, url: src.url ?? null, ref: src.ref ?? null, addedAt: src.addedAt?.toISOString() ?? '' })),
    artifacts: s.artifacts.map((a) => ({ id: a.id, kind: a.kind, title: a.title, content: a.content, createdAt: a.createdAt?.toISOString() ?? '' })),
    ...
    createdAt: (s as StudySpaceDocument & { createdAt?: Date }).createdAt?.toISOString() ?? '',
  };
}
...
export class SpacesController {
  ...
  private assertEnabled(): void {
    if (this.config.get<{ studySpaces?: boolean }>('flags')?.studySpaces === false) {
      throw new ForbiddenException('Study Spaces is disabled on this deployment.');
    }
  }
  ...
  @Patch(':id')
  async update(@CurrentUser() user: AuthUser, @Param('id') id: string, @Body() dto: UpdateSpaceDto) {
    return toView(await this.spaces.update(user.id, id, dto));
  }

  @Post(':id/sources')
  async addSource(@CurrentUser() user: AuthUser, @Param('id') id: string, @Body() dto: AddSourceDto) {
    return toView(await this.spaces.addSource(user.id, id, dto));
  }

  @Delete(':id/sources/:sourceId')
  async removeSource(@CurrentUser() user: AuthUser, @Param('id') id: string, @Param('sourceId') sourceId: string) {
    return toView(await this.spaces.removeSource(user.id, id, sourceId));
  }

  @Post(':id/ask')
  ask(@CurrentUser() user: AuthUser, @Param('id') id: string, @Body() dto: AskSpaceDto) {
