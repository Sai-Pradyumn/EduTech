...
export class SpacesService {
  constructor(
    @InjectModel(StudySpace.name)
    private readonly model: Model<StudySpaceDocument>,
  ...
  async ask(
    ...
    id: string,
    question: string,
  ): Promise<{ answer: string; usedSources: string[] }> {
    const s = await this.get(userId, id);
    ...
    if (!context) {
      return {
        answer:
          'This space has no readable sources yet. Add notes, a transcript or pasted text first.',
        usedSources: [],
      };
    }
    let answer: string;
    try {
      answer = await this.ai.generateText(
        [
          {
            role: 'system',
            content:
              'Answer ONLY from the provided sources. Cite the source titles you used. If the answer is not in the sources, say so plainly.',
          },
          {
            role: 'user',
            content: `Sources:\n${context}\n\nQuestion: ${question}`,
        ...
        {
          meta: { userId, agentType: AgentType.Rag, operation: 'space.ask' },
          temperature: 0.3,
        },
      );
    } catch {
    ...
    return {
      answer,
      usedSources: s.sources.filter((src) => src.text).map((src) => src.title),
    };
  }
  ...
  async flashcards(userId: string, id: string): Promise<StudySpaceDocument> {
    const s = await this.get(userId, id);
    const cards = s.sources
      .slice(0, 8)
      .map(
        (src, i) =>
          `**Q${i + 1}.** What is the key point of "${src.title}"?\n\n> ${this.firstSentences(src.text || src.title, 1)}`,
      );
  ...
  async audioOverview(
    userId: string,
    id: string,
  ): Promise<{ space: StudySpaceDocument; script: string }> {
    const s = await this.get(userId, id);
  ...
  async createFlow(
    userId: string,
    id: string,
  ): Promise<{ space: StudySpaceDocument; flowId: string }> {
    const s = await this.get(userId, id);
    const flow = await this.flows.generate(userId, {
      goal: `Learn ${s.title}`,
    });
    if (!s.linkedFlowIds.includes(String(flow._id)))
      s.linkedFlowIds.push(String(flow._id));
    await s.save();
  ...
  async createQuiz(
    userId: string,
    id: string,
  ): Promise<{ space: StudySpaceDocument; quizId: string }> {
    const s = await this.get(userId, id);
    const quiz = await this.assessment.generate(userId, {
      source: 'topic',
      topic: s.title.slice(0, 110),
      difficulty: Difficulty.Intermediate,
    });
    if (!s.linkedQuizIds.includes(String(quiz._id)))
      s.linkedQuizIds.push(String(quiz._id));
    await s.save();
  ...
  async createVisual(
    userId: string,
    id: string,
  ): Promise<{ space: StudySpaceDocument; visualId: string }> {
    const s = await this.get(userId, id);
    ...
    if (!s.linkedVisualIds.includes(String(visual._id)))
      s.linkedVisualIds.push(String(visual._id));
    await s.save();
  ...
  private upsertArtifact(
    ...
    if (existing) {
      ...
      s.artifacts.push({
        id: `art_${kind}_${Date.now().toString(36)}`,
        kind,
        title,
        content,
        createdAt: new Date(),
  ...
  private deterministicSummary(s: StudySpaceDocument): string {
    if (!s.sources.length)
      return 'This space is empty — add sources to summarize.';
    const points = s.sources
      .slice(0, 6)
      .map(
        (src) =>
          `- **${src.title}**: ${this.firstSentences(src.text || src.title, 1)}`,
      );
