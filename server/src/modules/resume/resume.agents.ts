        ...
        [
          {
            role: 'system',
            content:
              'Write a 2-sentence first-person-free resume summary for the given candidate. Concrete, ATS-friendly, no buzzword soup, no markdown.',
          },
          {
            role: 'user',
            content: `Target role: ${role}. Skills: ${skills.join(', ')}. Verified events: ${proofCount}. Projects: ${projectCount}.`,
          },
        ],
        {
          temperature: 0.5,
  ...
  async tailor(
    ...
    const fallback: JdAnalysis = {
      ...
      coverLetter:
        ...
        (missing.length
          ? `I'm actively closing the gaps you value most (${missing.slice(0, 3).join(', ')}) through focused, daily practice.\n\n`
          : '') +
      ...
      const out = await this.ai.generateStructuredOutput<JdAnalysis>(
        [
          {
            role: 'system',
            content:
              'Write a short, sincere cover letter (<= 140 words) and a 1-sentence tailored summary for this candidate and role. Honest about gaps. Return JSON {coverLetter, tailoredSummary}. No markdown.',
          },
          {
            role: 'user',
            content: `Candidate: ${name}. Company: ${company}. Role: ${role}. Matched skills: ${matched.join(', ')}. Missing: ${missing.join(', ')}.`,
          },
        ],
        {
          type: 'object',
        ...
        },
        {
          ...
          meta: {
            userId,
            agentType: AgentType.Career,
            operation: 'application.tailor',
          },
          mockFactory: () => fallback,
        },
      );
      return {
        coverLetter: out.coverLetter || fallback.coverLetter,
        tailoredSummary: out.tailoredSummary || fallback.tailoredSummary,
      };
    } catch (err) {
