  ...
  async analyzeJd(userId: string, dto: AnalyzeJdDto): Promise<JdMatch> {
    ...
    const learnerSkills = new Set<string>(
      [
        ...
        ...pv.skills
          .filter((s) => s.mastery > 0 || s.evidenceCount > 0)
          .map((s) => s.skill),
    ...
    const tailored = await this.agent.tailor(
      userId,
      pv.identity.name,
      dto.company,
      dto.role,
    ...
    const prepPlan =
      (analysis.blockers[0]
        ? `Before applying: ${analysis.blockers[0].title} — ${analysis.blockers[0].impact} `
        : '') +
      (missing.length
        ? `Close these JD gaps fast: ${missing.slice(0, 4).join(', ')}. Build one project that uses them and add it to your passport.`
        : 'You match the core requirements — tailor your resume and apply.');
