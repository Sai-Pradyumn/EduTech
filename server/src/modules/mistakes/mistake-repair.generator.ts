...
const TYPE_CORRECTION: Record<MistakeType, string> = {
  misconception: 'You hold an incorrect mental model here — rebuild it from the definition up, with one worked example.',
  missing_prerequisite: 'A prerequisite is missing — patch the underlying concept before retrying this one.',
  careless_error: 'You know this but slipped — slow down and verify each step on the retry.',
  weak_recall: 'The idea is fuzzy — space out short recall reps until it is automatic.',
  poor_explanation: 'You can do it but cannot explain it — practice teaching it back out loud.',
  implementation_gap: 'Theory is fine but implementation is shaky — build a tiny project that forces this concept.',
  interview_communication_gap: 'Reasoning is right but communication is unclear — rehearse a crisp spoken answer.',
  project_architecture_gap: 'A design/architecture gap — review the pattern and re-draw the architecture.',
};
 ...
 */
export function buildRepairPlan(concept: string, type: MistakeType): { correction: string; actions: RepairAction[] } {
  const id = (k: string) => `${k}_${concept.replace(/[^a-z0-9]+/gi, '').slice(0, 12).toLowerCase()}`;
