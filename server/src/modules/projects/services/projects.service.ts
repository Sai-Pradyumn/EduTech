...
} from './project-blueprint.generator';
...
export class ProjectsService {
  ...
  async generate(
    ...
    source: ProjectSource = 'goal',
  ): Promise<ProjectDocument> {
    const profile = await this.profiles.findByUser(userId);
    const difficulty =
      dto.difficulty ?? this.difficultyFor(profile?.currentSkillLevel);
    const bp: Blueprint = this.blueprint.generate(
      dto.goal.trim(),
  ...
  async generateAiReview(userId: string, id: string): Promise<ProjectDocument> {
    ...
    if (!project.submission?.submittedAt)
      throw new BadRequestException(
        'Submit the project before requesting a review.',
      );
    return this.runAiReview(userId, project);
  }
  ...
  async mentorReview(
    ...
    if (!project.submission?.submittedAt)
      throw new BadRequestException('Project has not been submitted yet.');
  ...
  async stats(userId: string): Promise<ProjectStats> {
    ...
    const avgProgress = list.length
      ? Math.round(
          list.reduce((s, p) => s + p.progressPercentage, 0) / list.length,
        )
      : 0;
    return {
      total: list.length,
  ...
  private reviewSummary(
    ...
    const band =
      draft.overallScore >= 80
        ? 'strong'
        : draft.overallScore >= 60
          ? 'solid'
          : draft.overallScore >= 40
            ? 'a promising start'
            : 'an early draft';
    const lead = `${project.title} scores ${draft.overallScore}/100 — ${band}.`;
    const strength = draft.strengths[0] ?? 'The scope is well-defined.';
    const next = draft.improvements[0]
      ? ` Your highest-impact next step: ${draft.improvements[0].text.toLowerCase()}`
      : ' Focus on polish to finish strong.';
  ...
  private recompute(project: ProjectDocument): void {
    ...
    project.milestones.forEach((m, i) => {
      if (project.progressPercentage >= (thresholds[i] ?? 100))
        m.reached = true;
    });
  }
