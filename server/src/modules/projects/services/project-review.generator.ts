...
export class ProjectReviewGenerator {
  generate(project: ProjectDocument): ProjectReviewDraft {
    ...
    const lowerPhases = tasks
      .map((t) => `${t.phase} ${t.title}`.toLowerCase())
      .join(' ');
  ...
  private strengths(ctx: {
    ...
    const out: string[] = [];
    if (ctx.taskRatio >= 0.9)
      out.push(
        'Nearly all planned tasks were completed — strong follow-through.',
      );
    if (ctx.hasDemo)
      out.push('A live demo link makes the work immediately verifiable.');
    if (ctx.hasGithub)
      out.push('Source is published, so reviewers can inspect the code.');
    if (ctx.hasTests)
      out.push('Includes a testing phase — signals quality awareness.');
    if (ctx.phases >= 4)
      out.push(`Well-structured across ${ctx.phases} delivery phases.`);
    if (ctx.stack.length >= 3)
      out.push(`Uses a realistic stack (${ctx.stack.slice(0, 4).join(', ')}).`);
    if (out.length === 0)
      out.push(
        'Project scope is defined and submitted — a solid base to build on.',
      );
  ...
  private improvements(ctx: {
    ...
    if (!ctx.hasGithub)
      items.push({
        text: 'Publish the source to a public GitHub repo and add the link.',
        severity: 'high',
      });
    if (!ctx.hasDemo)
      items.push({
        text: 'Deploy a live demo (Vercel/Netlify/Render) so reviewers can try it.',
        severity: 'high',
      });
    if (ctx.taskRatio < 0.8)
      items.push({
        text: 'Finish the remaining planned tasks before considering it done.',
        severity: 'high',
      });
    if (!ctx.hasTests)
      items.push({
        text: 'Add automated tests for the core flows to demonstrate reliability.',
        severity: 'medium',
      });
    if (!ctx.hasCi)
      items.push({
        text: 'Set up CI (lint + tests on push) for a professional workflow.',
        severity: 'medium',
      });
    if (ctx.notesLen < 80)
      items.push({
        text: 'Write a richer README/notes section: problem, approach, and how to run it.',
        severity: 'medium',
      });
    if (!ctx.hasVideo)
      items.push({
        text: 'Record a short walkthrough video — great for portfolios and interviews.',
        severity: 'low',
      });
    if (items.length === 0)
      items.push({
        text: 'Polish UI details and edge-case handling to push from good to great.',
        severity: 'low',
      });
