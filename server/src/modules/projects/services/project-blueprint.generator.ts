...
export class ProjectBlueprintGenerator {
  generate(goal: string, skills: string[], difficulty: Difficulty): Blueprint {
    ...
    push(
      'Setup',
      `Scaffold the ${stack[0]} + ${stack[1]} project`,
      `Initialize the repo, ${language} tooling, and a running dev server.`,
      2,
    );
    push(
      'Setup',
      'Define the data model & API contract',
      'Sketch entities, routes and the response shape before coding.',
      2,
    );

    // Core phase — from the archetype.
    for (const t of arch.coreTasks)
      push('Core features', t.title, t.description, 4);

    // Conditional phases by difficulty.
    if (phases.includes('Data & state')) {
      push(
        'Data & state',
        'Wire persistent storage + loading/error states',
        `Connect ${stack[2]} and handle async UI states.`,
        3,
      );
    }
    if (phases.includes('Auth')) {
      push(
        ...
        'JWT (or session) auth with guards on the API and UI.',
        4,
      );
    }
    push(
      'Polish',
      'Empty states, validation & responsive layout',
      'Make it robust and good on mobile.',
      3,
    );
    if (phases.includes('Testing')) {
      push(
        'Testing',
        'Write tests for the core flow',
        'Unit + a couple of integration tests for the happy path and edge cases.',
        3,
      );
    }
    if (phases.includes('CI/CD')) {
      push(
        'CI/CD',
        'Set up CI + automated deploy',
        'Lint/test on push and deploy on green.',
        3,
      );
    }
    push(
      'Deploy',
      'Deploy + write the README',
      'Ship it publicly and document setup, decisions and a demo link.',
      2,
    );
    ...
    return {
      ...
      summary: `A ${difficulty} ${isKnown ? title.toLowerCase() : 'project'} built with ${stack.join(' · ')}. ${
        isKnown
          ? 'Covers the real-world features an interviewer expects.'
          : 'Scoped to ship a portfolio-worthy build.'
    ...
    const ms: BlueprintMilestone[] = [
      {
        ...
        description: 'Project runs end-to-end with one feature working.',
        criteria: ['Dev server runs', 'One core feature works end-to-end'],
        reached: false,
      },
      {
        title: 'Feature complete',
        description: 'All core features built.',
        criteria: features.slice(0, 3).map((f) => `${f} done`),
        reached: false,
      },
      {
        title: 'Shipped',
        description: 'Deployed, documented and demo-ready.',
        criteria: [
          'Deployed publicly',
          'README + demo link',
