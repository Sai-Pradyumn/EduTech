...
import {
  ...
  ReadinessAnalysis,
} from '../career-readiness/career-readiness.service';
import {
  CouncilRecommendation,
  CouncilRecommendationDocument,
...
export class OutcomeCouncilService {
  constructor(
    @InjectModel(CouncilRecommendation.name)
    private readonly model: Model<CouncilRecommendationDocument>,
  ...
  async recommend(userId: string): Promise<CouncilResult> {
    const [analysis, twin] = await Promise.all([
      this.readiness.analyze(userId),
  ...
  async latest(userId: string): Promise<CouncilResult | null> {
    ...
    return {
      ...
      context: (doc.context as {
        role: string;
        readinessScore: number;
        band: string;
      }) ?? { role: '', readinessScore: 0, band: 'early' },
      generatedAt:
  ...
  private gather(
    a: ReadinessAnalysis,
    twin: {
      weaknessRoots: { concept: string; severity: number }[];
      retentionRisk: number;
    },
  ): CouncilAction[] {
    const out: CouncilAction[] = [];
    const dim = (k: string) =>
      a.dimensions.find((d) => d.key === k)?.score ?? 0;
    const topGap = a.skillGaps
      .filter((g) => !g.met)
    ...
    if (topGap) {
      out.push({
        ...
        expectedImpact: clamp(
          45 +
            Math.round(topGap.gap * 0.45) -
            Math.round(
              a.dimensions.find((d) => d.key === 'skills')!.score * 0.1,
            ),
        ),
    ...
    if (!a.interviewGap.met) {
      out.push({
        ...
        why: a.interviewGap.score
          ? `Your interview score is ${a.interviewGap.score}/100 — push it higher.`
          : 'You have no interview evidence yet — get a baseline.',
