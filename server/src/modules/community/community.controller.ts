...
import { CreateChannelDto, CreateReplyDto, CreateThreadDto } from './dto/community.dto';
...
export class CommunityController {
  ...
  createChannel(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Body() dto: CreateChannelDto) {
    return this.community.createChannel(this.orgOrThrow(ctx), user.id, dto);
  }

  @Get('channels/:id/threads')
  async threads(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Param('id') channelId: string) {
  ...
  @Post('threads')
  @HttpCode(HttpStatus.CREATED)
  async createThread(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Body() dto: CreateThreadDto) {
    await this.assertChannelOrg(ctx, dto.channelId);
    const author = await this.users.findByIdOrThrow(user.id);
    return this.community.createThread(this.orgOrThrow(ctx), user.id, author.name, dto);
  }

  @Get('threads/:id')
  async thread(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Param('id') id: string) {
    await this.assertThreadOrg(ctx, id);
    return this.community.getThread(id, user.id);
  }

  @Post('threads/:id/upvote')
  async upvoteThread(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Param('id') id: string) {
    await this.assertThreadOrg(ctx, id);
    return this.community.toggleThreadUpvote(id, user.id);
  }

  @Delete('threads/:id')
  async deleteThread(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Param('id') id: string) {
    await this.assertThreadOrg(ctx, id);
  ...
  @HttpCode(HttpStatus.CREATED)
  async reply(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Param('id') id: string, @Body() dto: CreateReplyDto) {
    await this.assertThreadOrg(ctx, id);
    const author = await this.users.findByIdOrThrow(user.id);
  ...
  acceptAnswer(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Param('id') id: string) {
    return this.community.acceptAnswer(id, user.id, this.canModerate(ctx));
  }

  @Delete('replies/:id')
  deleteReply(@CurrentUser() user: AuthUser, @CurrentOrg() ctx: OrgContext, @Param('id') id: string) {
  ...
  private async assertChannelOrg(ctx: OrgContext, channelId: string): Promise<void> {
    if (ctx.isPlatform) return;
    const orgId = await this.community.orgIdOfChannel(channelId);
    if (ctx.organizationId !== orgId) throw new ForbiddenException('You do not have access to this channel.');
  }

  private async assertThreadOrg(ctx: OrgContext, threadId: string): Promise<void> {
    if (ctx.isPlatform) return;
    const orgId = await this.community.orgIdOfThread(threadId);
    if (ctx.organizationId !== orgId) throw new ForbiddenException('You do not have access to this thread.');
  }
}
