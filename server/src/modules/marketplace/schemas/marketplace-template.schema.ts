...
export const TEMPLATE_STATUS = ['draft', 'pending_review', 'published', 'rejected'] as const;
...
export class MarketplaceTemplate {
  @Prop({ type: Types.ObjectId, ref: 'User', required: true, index: true }) creator!: Types.ObjectId;
  @Prop({ type: Types.ObjectId, ref: 'Organization' }) organization?: Types.ObjectId;

  @Prop({ type: String, enum: TEMPLATE_TYPES, required: true, index: true }) type!: TemplateType;
  ...
  @Prop({ type: String, enum: ['public', 'org'], default: 'public' }) visibility!: 'public' | 'org';
  @Prop({ type: String, enum: TEMPLATE_STATUS, default: 'draft', index: true }) status!: TemplateStatus;

  @Prop({ default: 0 }) usageCount!: number;
  @Prop({ type: { avg: Number, count: Number }, default: () => ({ avg: 0, count: 0 }) }) ratingSummary!: { avg: number; count: number };
