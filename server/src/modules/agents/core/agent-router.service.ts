...
const CLASSIFY_SCHEMA = {
  ...
  properties: {
    ...
    documentScoped: { type: 'boolean', description: 'true if the question is about the user’s uploaded documents' },
    confidence: { type: 'number', description: '0..1 confidence in the intent' },
  },
  ...
  async classify(message: string, hint?: Intent): Promise<Classification> {
    ...
    if (this.ai.isLive) {
      try {
        ...
        if (raw?.intent && Object.values(Intent).includes(raw.intent) && (raw.confidence ?? 0) >= 0.45) {
          return {
            intent: raw.intent,
            entities: {
    ...
    return { intent: this.classifyIntent(message), entities: {}, confidence: 0.5 };
  }
