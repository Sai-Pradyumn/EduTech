...
const TARGET: Record<
  CareerTarget,
  { label: string; bar: number; focus: string[] }
> = {
  [CareerTarget.Internship]: {
    label: 'an internship',
    bar: 70,
    focus: ['DSA basics', '1 solid project', 'resume', 'communication'],
  },
  [CareerTarget.FullTime]: {
    label: 'a full-time SDE role',
    bar: 85,
    focus: [
      'DSA depth',
      '2–3 strong projects',
  ...
  [CareerTarget.Startup]: {
    label: 'a startup role',
    bar: 80,
    focus: [
      'full-stack breadth',
      'shipping speed',
  ...
  [CareerTarget.HigherStudies]: {
    label: 'higher studies',
    bar: 80,
    focus: ['fundamentals depth', 'research/projects', 'GRE/exam prep', 'SOP'],
  },
  [CareerTarget.SkillImprovement]: {
    label: 'leveling up your skills',
    bar: 75,
    focus: [
      'consistent practice',
      'projects',
...
export class CareerAgentService implements IAgent {
  ...
  async handle(ctx: AgentRuntimeContext): Promise<AgentResponse> {
    ...
    const target =
      TARGET[profile?.careerTarget ?? CareerTarget.SkillImprovement];
    const baseline =
      BASELINE[profile?.currentSkillLevel ?? SkillLevel.Beginner];

    ctx.emit({
      type: 'thinking',
      messageId: '',
      label: `Mapping your skills against ${target.label}`,
    });
    ctx.emit({
      type: 'tool_call',
      messageId: '',
      tool: 'career.gap',
      label: 'Computing skill gaps & readiness',
    });
    ...
    const readiness = skills.length
      ? Math.round(
          skills.reduce(
            (s, k) => s + Math.min(100, (k.current / k.target) * 100),
            0,
          ) / skills.length,
        )
      : baseline;
    ...
    ctx.emit({
      ...
      messageId: '',
      tool: 'career.gap',
      summary: `Readiness ~${readiness}% for ${target.label}`,
    });

    const gapBlock: SkillGapBlock = {
      type: 'skill_gap',
      title: `Skill gap → ${target.label}`,
      skills,
    };
    const planBlock: StudyPlanBlock = {
      type: 'study_plan',
      title: 'Your 4-step readiness plan',
      items: target.focus.map((f, i) => ({
        label: `${i + 1}. ${this.titleCase(f)}`,
        kind: 'career',
      })),
    } satisfies VisualBlock;

    const fallback = this.buildAnswer(
    ...
    return {
      ...
      actions: [
        {
          id: 'interview',
          label: 'Mock interview me',
          kind: 'ask_interviewer',
        },
        {
          id: 'resume',
          label: 'Review my resume bullets',
          kind: 'custom',
          payload: { topic: 'resume' },
        },
        {
          id: 'project',
          label: 'Suggest a portfolio project',
          kind: 'open_route',
          payload: { route: '/app/projects' },
        },
      ],
      ...
      recommendedNextActions: [
        skills[0]
          ? `Close your biggest gap first: ${skills.sort((a, b) => b.target - b.current - (a.target - a.current))[0].skill}`
          : 'Add your skills in your profile to sharpen this map',
  ...
  private buildSkills(
    ...
    for (const s of current.slice(0, 5))
      skills.set(this.titleCase(s), baseline);
    ...
    if (skills.size === 0) {
      [
        ...
        'Projects',
        'Communication',
      ].forEach((s) => skills.set(s, baseline));
    }
    return [...skills.entries()].slice(0, 6).map(([skill, val]) => ({
      skill: this.short(skill),
      current: val,
      target: bar,
    }));
  }
  ...
  private buildAnswer(
    ...
    const band =
      readiness >= 75
        ? 'you’re close — tighten the gaps and start applying'
        : readiness >= 50
          ? 'solid base — a focused push gets you there'
          : 'early days — the plan below is your fastest route';
    const biggest = [...skills].sort(
      (a, b) => b.target - b.current - (a.target - a.current),
    )[0];
    return [
