...
export class AdminInsightAgentService implements IAgent {
  ...
  async handle(ctx: AgentRuntimeContext): Promise<AgentResponse> {
    ctx.emit({ type: 'thinking', messageId: '', label: 'Aggregating platform AI usage' });
    ctx.emit({ type: 'tool_call', messageId: '', tool: 'admin.usage', label: 'Reading ai_usage_logs' });

    const usage = await this.ai.usageSummary();
    ctx.emit({ type: 'tool_result', messageId: '', tool: 'admin.usage', summary: `${usage.totalCalls} calls across ${usage.byAgent.length} agents` });
    ...
    const block: AdminInsightBlock = {
      ...
      metrics: [
        ...
        { label: 'Tokens (est.)', value: usage.totalTokens, hint: 'in + out, placeholder counts' },
        { label: 'Avg latency', value: `${usage.avgLatencyMs}ms` },
        { label: 'Top agent', value: topAgent ? topAgent.agentType : '—', hint: topAgent ? `${topAgent.count} calls` : undefined },
      ],
      notes: usage.byAgent.slice(0, 6).map((a) => `${a.agentType}: ${a.count} call(s)`),
    };
    ...
    const answer = [
      ...
      topAgent ? `- Busiest agent: **${topAgent.agentType}** (${topAgent.count})` : '',
      '',
    ...
    return {
      ...
      recommendedNextActions: ['Open the Admin Command Center for the full picture'],
    };
  }
