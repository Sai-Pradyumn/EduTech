...
function classify(message: string): {
  ...
  if (/undefined|null|cannot read|is not a function|nan/.test(m)) {
    return {
      ...
      causes: [
        'A value is missing/undefined when you use it',
        'Async data not loaded yet',
        'A typo in a property name',
      ],
      checks: [
        'Log the variable right before the failing line',
        'Check the value exists before accessing its property',
        'Confirm async data arrived (await / loading state)',
      ],
    };
  }
  if (/cors|network|fetch|axios|404|500|api|request failed/.test(m)) {
    return {
      ...
      checks: [
        'Open the Network tab — what status + response body?',
        'Verify the URL, method and headers',
        'Reproduce the request with curl/Postman',
      ],
    };
  }
  if (/css|style|layout|align|center|flex|grid|not showing|overflow/.test(m)) {
    return {
      ...
      causes: [
        'A parent constrains size/overflow',
        'Wrong display/position context',
        'Specificity overriding your rule',
      ],
      checks: [
        'Inspect the element — which rule actually applies?',
        'Add a temporary outline to see box bounds',
        'Check the parent’s display/overflow',
      ],
    };
  }
  if (
    /build|compile|module not found|cannot find|import|type error|ts\d/.test(m)
  ) {
    return {
      ...
      checks: [
        'Read the FIRST error (later ones often cascade)',
        'Verify the import path & that the package is installed',
        'Clear the cache / reinstall and rebuild',
      ],
    };
  }
  return {
    kind: 'a logic bug',
    causes: [
      'An assumption about the data is wrong',
      'Off-by-one / wrong condition',
      'State not updating as expected',
    ],
    checks: [
      'Restate what you expect vs what happens',
      'Add logs at each step to find where they diverge',
      'Test the smallest input that reproduces it',
    ],
  };
}
...
export class DoubtSolverAgentService implements IAgent {
  ...
  async handle(ctx: AgentRuntimeContext): Promise<AgentResponse> {
    ...
    const reveal =
      ...
      /show me the fix|just tell me|reveal the (fix|answer)|give me the (answer|fix)/i.test(
        problem,
      );
    ...
    ctx.emit({
      type: 'thinking',
      messageId: '',
      label: 'Reading the error & narrowing the cause',
    });
    ctx.emit({
      type: 'tool_call',
      messageId: '',
      tool: 'doubt.classify',
      label: `Looks like ${c.kind}`,
    });
    ctx.emit({
      type: 'tool_result',
      messageId: '',
      tool: 'doubt.classify',
      summary: reveal
        ? 'Revealing a direct approach'
        : 'Hint-first (you debug it)',
    });

    const fallback = reveal
      ? this.revealAnswer(c, problem)
      : this.hintAnswer(c, problem, name);
    const system =
    ...
    return {
      ...
      confidence: 0.85,
      followUpQuestions: [
        'Here’s what I tried and the new error…',
        'Why did that cause the bug?',
      ],
