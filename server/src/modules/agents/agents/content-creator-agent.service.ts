...
export class ContentCreatorAgentService implements IAgent {
  ...
  async handle(ctx: AgentRuntimeContext): Promise<AgentResponse> {
    ...
    ctx.emit({ type: 'thinking', messageId: '', label: `Drafting ${format} on ${topic}` });
    ...
    const plan: StudyPlanBlock = {
      ...
      items: [
        ...
        { label: `Recall ${topic} from memory (no peeking)`, minutes: 10, kind: 'revision' },
        { label: `Apply it: one small exercise on ${topic}`, minutes: 20, kind: 'practice' },
      ],
    };
    blocks.push(plan);
    if (format === 'flashcards') blocks.push(this.flashcardQuiz(topic));
    for (const b of blocks) ctx.emit({ type: 'visual_block', messageId: '', block: b });
    ...
    return {
      ...
      answer,
      actions: [
        { id: 'flash', label: 'Make flashcards', kind: 'custom', payload: { topic, format: 'flashcards' } },
        { id: 'quiz', label: 'Quiz me on this', kind: 'generate_quiz', payload: { topic } },
        { id: 'simpler', label: 'Explain simpler', kind: 'simpler', payload: { topic } },
      ],
      visualBlocks: blocks,
      confidence: 0.84,
      followUpQuestions: [`Make a cheat-sheet for ${topic}`, `Turn this into flashcards`, `Give me practice questions on ${topic}`],
      recommendedNextActions: [`Test yourself on ${topic} in the Quiz Studio`, 'Revisit these notes tomorrow (spaced repetition)'],
    };
  }
  ...
  private buildContent(format: Format, topic: string, level: string): string {
    ...
    if (format === 'summary') {
      return [`### ${t} — TL;DR`, '', `**In one line:** ${t} is a ${level}-level concept worth knowing because it shows up in real work and interviews.`, '', '**Key points**', `- The core idea and the problem it solves`, `- How it works, step by step`, `- When to use it (and when not to)`, `- A common mistake to avoid`, '', '_Want the full notes or flashcards? Just ask._'].join('\n');
    }
    if (format === 'cheatsheet') {
      return [`### ${t} — Cheat-sheet`, '', '| Thing | Quick reference |', '|---|---|', `| What | The essence of ${t} in a sentence |`, '| Syntax / shape | The canonical form you’ll write |', '| Gotchas | The 1–2 mistakes everyone makes |', '| When | The situations it’s the right tool for |', '', `_Pin this; it’s the 80/20 of ${t}._`].join('\n');
    }
    if (format === 'flashcards') {
      return [`### ${t} — Flashcards`, '', `**Q1.** What is ${t} in one sentence?`, `**A1.** The core definition.`, '', `**Q2.** When would you use ${t}?`, `**A2.** The main use-case + a trade-off.`, '', `**Q3.** What’s a common mistake with ${t}?`, `**A3.** The pitfall and how to avoid it.`, '', '_Use the recall quiz on the right — cover the answers first._'].join('\n');
    }
    return [
      `### ${t} — Study notes`,
      '',
      ...
      questions: [
        {
          ...
          options: [`Its core idea, stated precisely`, 'A vaguely related concept', 'An unrelated tool', 'None of these'],
          answerIndex: 0,
  ...
  private extractTopic(message: string): string {
    const cleaned = message
      .toLowerCase()
      .replace(/^(can you |please )?(generate|make|create|write|give me)\s+/i, '')
      .replace(/\b(notes|flashcards|flash cards|a summary|summary|cheat ?sheet|study (notes|material))\b/gi, '')
