...
import { buildQuestions, INTERVIEW_TYPE_META, InterviewType } from './interview-bank';
...
export class InterviewService {
  constructor(
    @InjectModel(InterviewSession.name) private readonly model: Model<InterviewSessionDocument>,
  ...
  async start(userId: string, type: InterviewType, roleId?: string): Promise<InterviewSessionDocument> {
    const profile = await this.profiles.findByUser(userId);
    const role = roleId ? findRole(roleId)?.title ?? '' : matchRoleFromGoal(profile?.mainGoal ?? '').title;
    const questions = buildQuestions(type, role).map((q) => ({ id: randomUUID(), question: q, answer: '', feedback: '', answered: false }));
    return this.model.create({ user: new Types.ObjectId(userId), type, role, questions, currentIndex: 0, status: 'active' });
  }

  async respond(userId: string, sessionId: string, answer: string): Promise<InterviewSessionDocument> {
    const session = await this.owned(userId, sessionId);
    if (session.status === 'finished') throw new BadRequestException('This interview is already finished.');
    const q = session.questions[session.currentIndex];
    if (!q) throw new BadRequestException('No more questions — finish the interview.');
  ...
  async finish(userId: string, sessionId: string): Promise<InterviewSessionDocument> {
    const session = await this.owned(userId, sessionId);
    if (session.status === 'finished') return session;
    const answered = session.questions.filter((q) => q.answered && typeof q.score === 'number');
    const avg = answered.length ? Math.round(answered.reduce((s, q) => s + (q.score ?? 0), 0) / answered.length) : 0;
    const completion = Math.round((answered.length / Math.max(1, session.questions.length)) * 100);

    session.technicalScore = avg;
    session.communicationScore = clamp(Math.round(avg * 0.9 + completion * 0.1));
    session.confidenceScore = clamp(Math.round(completion * 0.5 + avg * 0.5));
    session.overallScore = clamp(Math.round(session.technicalScore * 0.5 + session.communicationScore * 0.25 + session.confidenceScore * 0.25));

    session.strengths = answered.filter((q) => (q.score ?? 0) >= 75).map((q) => this.shortLabel(q.question)).slice(0, 3);
    const weak = answered.filter((q) => (q.score ?? 0) < 60).map((q) => this.shortLabel(q.question));
    ...
    session.summary =
      ...
      (passed ? 'Strong performance — you cleared the bar.' : 'Below the bar — drill the weak areas and retry.');
    ...
    for (const concept of session.weakConcepts) {
      await this.mistakes.captureManual(userId, { concept: `Interview: ${concept}`, severity: 62, source: 'manual' }).catch(() => undefined);
    }
    return session;
  }

  list(userId: string): Promise<InterviewSessionDocument[]> {
    return this.model.find({ user: new Types.ObjectId(userId) }).sort({ createdAt: -1 }).limit(40).exec();
  }
  ...
  private async owned(userId: string, id: string): Promise<InterviewSessionDocument> {
    const session = await this.model.findOne({ _id: new Types.ObjectId(id), user: new Types.ObjectId(userId) }).exec();
    if (!session) throw new NotFoundException('Interview session not found');
    return session;
  }

  private shortLabel(q: string): string {
    const cleaned = q.replace(/[?.].*$/, '').replace(/^(tell me about|how do you|how would you|explain|describe|walk me through|what'?s|what is|design)\s+/i, '');
