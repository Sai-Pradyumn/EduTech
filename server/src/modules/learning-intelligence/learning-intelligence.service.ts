...
export class LearningIntelligenceService {
  ...
  async overview(userId: string): Promise<LearningIntelligence> {
    const [profile, roadmap, stats, attempts, mastery, projectStats, sessionList] = await Promise.all([
    ...
    const activityDates = [
      ...
      ...sessionList.map((s) => (s.lastMessageAt ?? (s as { updatedAt?: Date }).updatedAt)?.toISOString?.() ?? ''),
    ].filter(Boolean);
    ...
    const weakControl = weaknesses.length
      ? Math.round(100 - weaknesses.reduce((s, w) => s + w.severity, 0) / weaknesses.length)
      : hasQuiz
        ? 90
        : 60;
    ...
    const strengths = mastery.filter((m) => m.mastery >= 80).map((m) => this.titleCase(m.topic)).slice(0, 5);
    ...
    const skillCoverage = radar.length
      ? Math.round(radar.reduce((s, a) => s + Math.min(100, (a.value / a.target) * 100), 0) / radar.length)
    ...
    const projectScore = projectStats.total
      ? Math.min(100, projectStats.completed * 50 + projectStats.avgProgress * 0.5)
      : 0;
    ...
    const scores: ScoreCard[] = [
      { label: 'Roadmap progress', value: roadmapProgress, hint: roadmap ? `${roadmap.completedWeeks.length}/${roadmap.weeklyPlan.length} weeks` : 'No active roadmap' },
      { label: 'Quiz average', value: quizAvg, hint: `${stats.attempts} attempt${stats.attempts === 1 ? '' : 's'}` },
      { label: 'Consistency', value: activityScore, hint: `${activeDays} active day${activeDays === 1 ? '' : 's'} / 14` },
      { label: 'Weak-area control', value: weakControl, hint: weaknesses.length ? `${weaknesses.length} to improve` : 'No gaps flagged' },
    ];
    ...
    return {
      ...
      recommendations: this.recommendations(profile, roadmap, weaknesses, stats.attempts, projectStats.total),
    };
  }
  ...
  private radar(
    ...
    const baseline = SKILL_BASELINE[profile?.currentSkillLevel ?? SkillLevel.Beginner];
    const axes = new Map<string, number>();

    // Real quiz mastery first (strongest signal).
    for (const m of mastery.slice(0, 6)) axes.set(this.titleCase(m.topic), m.mastery);
    ...
    for (const week of roadmap?.weeklyPlan ?? []) {
      ...
      if (key && !axes.has(key)) axes.set(key, Math.max(baseline, roadmap?.progressPercentage ?? 0));
    }
    return [...axes.entries()].map(([label, value]) => ({ label: this.short(label), value, target }));
  }
  ...
  private weaknesses(
    ...
    for (const m of mastery) {
      if (m.mastery < 60) {
        out.push({ topic: this.titleCase(m.topic), severity: 100 - m.mastery, note: `Quiz mastery ${m.mastery}%` });
  ...
  private timeline(
    ...
    for (const a of attempts.slice(0, 6)) {
      items.push({ kind: 'quiz', label: `Scored ${a.score}% on a quiz`, at: a.createdAt });
    }
    for (const s of sessions.slice(0, 6)) {
      const at = s.lastMessageAt ? new Date(s.lastMessageAt).toISOString() : '';
      if (at) items.push({ kind: 'chat', label: s.title || 'AI session', detail: s.agentType, at });
    }
    if (roadmap) {
      items.push({ kind: 'roadmap', label: `${roadmap.completedWeeks.length} week(s) completed`, detail: roadmap.title, at: '' });
    }
    return items
      .filter((i) => i.at)
  ...
  private recommendations(
    ...
    const recs: string[] = [];
    if (weaknesses[0]) recs.push(`Drill your weakest area: ${weaknesses[0].topic} (take a weak-area quiz).`);
    if (attempts === 0) recs.push('Take your first quiz to calibrate your skill map.');
    if (projects === 0) recs.push('Build a portfolio project in the Project Studio to lift readiness.');
    if (roadmap) {
      const nextWeek = roadmap.weeklyPlan.find((w) => !roadmap.completedWeeks.includes(w.weekNumber));
      if (nextWeek) recs.push(`Continue your roadmap: Week ${nextWeek.weekNumber} — ${nextWeek.focus}.`);
    } else if (profile) {
      recs.push('Generate a roadmap to structure your learning.');
    }
    if (recs.length < 3) recs.push('Ask the AI Tutor to explain a concept you find shaky.');
    return recs.slice(0, 4);
  }

  // ── scoring helpers ──────────────────────────────────────────────────────────
  private blend(parts: [number, number][]): number {
    const total = parts.reduce((s, [v, w]) => s + Math.max(0, Math.min(100, v)) * w, 0);
  ...
  private streak(dates: string[]): number {
    const days = new Set(
      dates.map((d) => new Date(d).toISOString().slice(0, 10)).filter((s) => s.length === 10),
    );
    let streak = 0;
  ...
  private headline(
    ...
    if (!profile) return 'Complete onboarding to unlock your learning intelligence.';
    const name = profile.fullName?.split(' ')[0] ?? 'there';
    const band = health >= 75 ? 'on a strong trajectory' : health >= 50 ? 'making steady progress' : 'just getting started';
