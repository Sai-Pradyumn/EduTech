...
function toView(v: VisualAssetDocument) {
  return {
    ...
    createdAt:
      (
        v as VisualAssetDocument & { createdAt?: Date }
  ...
  private assertEnabled(): void {
    if (!this.isEnabled())
      throw new ForbiddenException(
        'Visual Studio is disabled on this deployment.',
      );
  }

  @Get('status')
  status() {
    const flags = this.config.get<{
      visualStudio?: boolean;
      imageGeneration?: boolean;
    }>('flags');
    return {
      enabled: flags?.visualStudio !== false,
      imageGeneration: flags?.imageGeneration === true,
    };
  }
