...
export class VisualsService {
  constructor(
    @InjectModel(VisualAsset.name)
    private readonly model: Model<VisualAssetDocument>,
  ...
  async generate(
    userId: string,
    dto: GenerateVisualDto,
  ): Promise<VisualAssetDocument> {
    ...
    return this.model.create({
      ...
      status: 'ready',
      provider:
        (v.metadata?.['imageProvider'] as string) ??
        (v.metadata?.['generator'] as string) ??
        'deterministic',
      metadata: v.metadata,
    });
  }
  ...
  async fromFlowNode(
    ...
    await this.flows
      .linkVisual(userId, dto.flowId, dto.nodeId, String(asset._id))
      .catch(() => undefined);
    return asset;
  }
  ...
  async get(userId: string, id: string): Promise<VisualAssetDocument> {
    if (!Types.ObjectId.isValid(id))
      throw new NotFoundException('Visual not found');
    const v = await this.model.findById(id).exec();
    if (!v || v.user.toString() !== userId)
      throw new NotFoundException('Visual not found');
    return v;
  }
  ...
  async regenerate(userId: string, id: string): Promise<VisualAssetDocument> {
    const v = await this.get(userId, id);
    const out = await this.explainer.generate(userId, {
      concept: v.prompt || v.title,
      type: v.type,
      level: v.level,
    });
