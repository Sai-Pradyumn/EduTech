...
export class SimulationsService {
  constructor(
    @InjectModel(Simulation.name) private readonly model: Model<SimulationDocument>,
  ...
  async get(userId: string, id: string): Promise<SimulationDocument> {
    if (!Types.ObjectId.isValid(id)) throw new NotFoundException('Simulation not found');
    const s = await this.model.findById(id).exec();
    if (!s || s.user.toString() !== userId) throw new NotFoundException('Simulation not found');
    return s;
  }

  async respond(userId: string, role: Role, id: string, message: string): Promise<SimulationDocument> {
    const sim = await this.get(userId, id);
    if (sim.status === 'finished') throw new NotFoundException('Simulation already finished');
  ...
  async finish(userId: string, id: string): Promise<SimulationDocument> {
    const sim = await this.get(userId, id);
    const userTurns = sim.transcript.filter((t) => t.role === 'user').map((t) => t.text);
    ...
    if (!sim.improvementPlan.length) sim.improvementPlan = [`Try a harder ${sim.type.replace('_', ' ')} on a new topic.`];
    ...
    if (score < 60) {
      try {
        const m = await this.mistakes.captureManual(userId, { concept: sim.topic, severity: 100 - score, source: 'manual' });
  ...
  async retry(userId: string, id: string, harder?: boolean): Promise<SimulationDocument> {
    const prev = await this.get(userId, id);
    const order: Difficulty[] = [Difficulty.Beginner, Difficulty.Intermediate, Difficulty.Advanced];
    const idx = order.indexOf(prev.difficulty);
    const next = harder ? order[Math.min(idx + 1, 2)] : order[Math.max(idx - 1, 0)];
    return this.start(userId, { type: prev.type as SimulationType, topic: prev.topic, difficulty: next });
  }

  /** Add a repair node for this simulation's topic to the active flow (or note none). */
  async createRepairFlow(userId: string, id: string): Promise<{ simulation: SimulationDocument; flowId: string | null; nodeId: string | null }> {
    const sim = await this.get(userId, id);
    const active = await this.flows.findActive(userId);
    if (!active) return { simulation: sim, flowId: null, nodeId: null };
    const { flow, nodeId } = await this.flows.addRepairNode(userId, String(active._id), sim.topic);
  ...
  private feedback(type: SimulationType, score: number, turns: number): string {
    ...
    return `${band} performance (${score}/100) across ${turns} response(s) in this ${type.replace('_', ' ')}. ${
      score >= 80 ? 'Keep this level and raise the difficulty.' : score >= 60 ? 'Tighten the weaker rubric areas and retry.' : 'Repair the fundamentals, then retry at an easier level.'
    }`;
  }
