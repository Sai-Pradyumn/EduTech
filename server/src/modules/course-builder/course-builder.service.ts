...
import { Course, CourseDocument, CourseVisibility } from './schemas/course.schema';
...
export class CourseBuilderService {
  ...
  async generate(userId: string, dto: GenerateCourseDto): Promise<CourseDocument> {
  ...
  async fromRoadmap(userId: string, roadmapId: string): Promise<CourseDocument> {
    ...
    if (!roadmap || roadmap.user.toString() !== userId) throw new NotFoundException('Roadmap not found');
    const outline = roadmap.weeklyPlan.map((w) => w.focus).join('\n');
    const bp = buildCourseBlueprint(roadmap.goal, (roadmap.difficulty as Difficulty) ?? Difficulty.Beginner, outline);
    return this.model.create({
      author: new Types.ObjectId(userId),
      ...
      description: bp.description,
      audience: 'Students',
      level: (roadmap.difficulty as Difficulty) ?? Difficulty.Beginner,
      source: 'roadmap',
      status: 'draft',
      visibility: 'private',
      modules: bp.modules,
  ...
  list(userId: string): Promise<CourseDocument[]> {
    return this.model.find({ author: new Types.ObjectId(userId) }).sort({ updatedAt: -1 }).exec();
  }

  async get(userId: string, id: string): Promise<CourseDocument> {
    if (!Types.ObjectId.isValid(id)) throw new NotFoundException('Course not found');
    const c = await this.model.findById(id).exec();
    if (!c || c.author.toString() !== userId) throw new NotFoundException('Course not found');
    return c;
  }

  async update(userId: string, id: string, dto: UpdateCourseDto): Promise<CourseDocument> {
      ...
      c.modules = dto.modules.map((m) => {
        ...
        return {
          ...
          lessons: (m.lessons ?? existing?.lessons ?? []).map((l) => ({
            ...
            title: l.title,
            content: l.content ?? '',
            estimateMinutes: existing?.lessons.find((x) => x.id === l.id)?.estimateMinutes ?? 20,
          })),
  ...
  async generateQuiz(userId: string, id: string, moduleId: string): Promise<CourseDocument> {
    const c = await this.get(userId, id);
    const mod = c.modules.find((m) => m.id === moduleId);
    if (!mod) throw new NotFoundException('Module not found');
    const quiz = await this.assessment.generate(userId, { source: 'topic', topic: mod.title.slice(0, 110), difficulty: c.level });
    mod.linkedQuizId = String(quiz._id);
    c.markModified('modules');
    return c.save();
  }

  async generateVisual(userId: string, id: string, moduleId: string): Promise<CourseDocument> {
    const c = await this.get(userId, id);
    const mod = c.modules.find((m) => m.id === moduleId);
    if (!mod) throw new NotFoundException('Module not found');
    const visual = await this.visuals.generate(userId, { concept: mod.title, type: 'mind_map', sourceType: 'manual' });
  ...
  async generateProject(userId: string, id: string): Promise<CourseDocument> {
    const c = await this.get(userId, id);
    const project = await this.projects.generate(userId, { goal: c.project.title || `Capstone for ${c.title}` });
    c.project.linkedProjectId = String(project._id);
    c.markModified('project');
    return c.save();
  }

  async generateFlow(userId: string, id: string): Promise<{ course: CourseDocument; flowId: string }> {
    const c = await this.get(userId, id);
  ...
  async publish(userId: string, role: Role, id: string, visibility: CourseVisibility): Promise<CourseDocument> {
    const c = await this.get(userId, id);
    if ((visibility === 'org' || visibility === 'cohort') && role !== Role.Mentor && role !== Role.Admin) {
      throw new ForbiddenException('Only mentors or admins can publish to an organization or cohort.');
    }
    c.status = 'published';
