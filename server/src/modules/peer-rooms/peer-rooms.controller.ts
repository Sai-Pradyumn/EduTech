...
import { CreateRoomDto, JoinRoomDto, PostMessageDto } from './dto/peer-room.dto';
...
function toView(r: PeerRoomDocument, userId: string) {
  return {
    ...
    messages: r.messages.map((m) => ({ id: m.id, name: m.name, kind: m.kind, text: m.text, at: m.at?.toISOString() ?? '', mine: m.user?.toString() === userId })),
    status: r.status,
    linkedFlowId: r.linkedFlowId ?? null,
    summary: r.summary,
    actionItems: r.actionItems,
    createdAt: (r as PeerRoomDocument & { createdAt?: Date }).createdAt?.toISOString() ?? '',
  };
}
...
export class PeerRoomsController {
  ...
  private assertEnabled(): void {
    if (this.config.get<{ peerRooms?: boolean }>('flags')?.peerRooms === false) {
      throw new ForbiddenException('Peer Rooms is disabled on this deployment.');
    }
  }
  ...
  async joinByCode(@CurrentUser() user: AuthUser, @Body() dto: JoinRoomDto) {
    this.assertEnabled();
    return toView(await this.rooms.joinByCode(user.id, user.email, user.role, dto.code), user.id);
  }
  ...
  async join(@CurrentUser() user: AuthUser, @Param('id') id: string) {
    return toView(await this.rooms.join(user.id, user.email, user.role, id), user.id);
  }

  @Post(':id/messages')
  async message(@CurrentUser() user: AuthUser, @Param('id') id: string, @Body() dto: PostMessageDto) {
    return toView(await this.rooms.postMessage(user.id, user.email, id, dto.text), user.id);
  }
