...
import { GeneratedQuestion, QuizGeneratorService } from './quiz-generator.service';
...
export class AssessmentService {
  ...
  async generate(userId: string, dto: GenerateQuizDto): Promise<QuizDocument> {
    ...
    const difficulty = dto.difficulty ?? (await this.adaptiveDifficulty(userId));
    ...
    switch (dto.source) {
      case 'document': {
        if (!dto.documentId) throw new BadRequestException('documentId is required for a document quiz.');
        const chunks = await this.knowledge.sampleChunks(userId, dto.documentId, Math.max(count, 8));
        if (chunks.length === 0) throw new BadRequestException('That document has no indexed content yet.');
        const doc = await this.knowledge.get(userId, dto.documentId);
        topic = doc.topic ?? doc.title;
        title = `Quiz · ${doc.title}`;
        questions = this.generator.fromDocument(topic, chunks, difficulty, count);
        documentId = new Types.ObjectId(dto.documentId);
        break;
      }
      case 'weak_area': {
        const profile = await this.profiles.findByUserOrThrow(userId);
        const weak = profile.weakAreas.length ? profile.weakAreas : [profile.mainGoal];
        topic = weak.join(', ');
      ...
      case 'roadmap': {
        ...
        if (!roadmap) throw new BadRequestException('No active roadmap to quiz from.');
        ...
        roadmapId = roadmap._id as Types.ObjectId;
        break;
      }
      default: {
        if (!dto.topic) throw new BadRequestException('topic is required for a topic quiz.');
    ...
    await this.ai.logUsage({ userId, agentType: AgentType.Assessment, operation: 'quiz.generate' });
    return quiz;
  }
  ...
  ): Promise<{ attempt: AttemptView; evaluation: Evaluation; review: ReviewQuestion[] }> {
  ...
  async topicMastery(userId: string): Promise<{ topic: string; mastery: number; answered: number }[]> {
    const attempts = await this.attempts
      .find({ user: new Types.ObjectId(userId) })
      .lean<QuizAttemptDocument[]>()
      .exec();
    const agg = new Map<string, { sevSum: number; n: number; answered: number }>();
  ...
  async stats(userId: string): Promise<{
    ...
    return {
      ...
      averageScore: avg,
      masteredTopics: ranked.filter((t) => t.severity === 0).map((t) => t.topic).slice(0, 6),
  ...
  private spread(topics: string[], difficulty: Difficulty, count: number): GeneratedQuestion[] {
  ...
  private currentWeek(roadmap: RoadmapDocument): { weekNumber: number; topics: string[] } | null {
    const done = new Set(roadmap.completedWeeks);
    const next = roadmap.weeklyPlan.find((w) => !done.has(w.weekNumber)) ?? roadmap.weeklyPlan[0];
  ...
  private async ownedQuiz(userId: string, id: string): Promise<QuizDocument> {
    if (!Types.ObjectId.isValid(id)) throw new NotFoundException('Quiz not found');
    const quiz = await this.quizzes.findOne({ _id: id, user: new Types.ObjectId(userId) });
  ...
  private toView(q: QuizDocument): QuizView {
    return {
      ...
      createdAt: (q as QuizDocument & { createdAt?: Date }).createdAt?.toISOString() ?? '',
    };
  }
  ...
  private toAttemptView(a: QuizAttemptDocument): AttemptView {
    return {
      ...
      feedback: a.feedback,
      createdAt: (a as QuizAttemptDocument & { createdAt?: Date }).createdAt?.toISOString() ?? '',
    };
  }
