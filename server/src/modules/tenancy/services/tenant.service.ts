...
export class TenantService {
  ...
  async resolve(
    ...
    const isPlatform =
      ...
      (!!user.platformRole && PLATFORM_ORG_ROLES.includes(user.platformRole));

    if (isPlatform) {
      return {
        organizationId:
          requestedOrgId ??
          (user.primaryOrganization ? String(user.primaryOrganization) : null),
        orgRole: user.platformRole ?? OrgRole.PlatformAdmin,
        permissions:
          ROLE_PERMISSIONS[user.platformRole ?? OrgRole.PlatformAdmin],
        isPlatform: true,
      };
    }
    ...
    const orgId =
      requestedOrgId ??
      (user.primaryOrganization ? String(user.primaryOrganization) : undefined);
    const membership = orgId
      ? await this.findMembership(authUser.id, orgId)
      : await this.memberships.findOne({
          user: new Types.ObjectId(authUser.id),
